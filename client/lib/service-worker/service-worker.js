...
const getResourceType = (request) => {
	...
	if (acceptHeader.indexOf('text/html') !== -1 || url.pathname.startsWith('/andromeda')) {
		return type.STATIC;
	}
...
self.addEventListener('install', (event) =>
	event.waitUntil(caches.open(type.STATIC).then((cache) => cache.addAll(opts.prefillAssets))),
);
